...
const mockUseAIChatFn = vi.fn();
vi.mock("ai/react", () => ({
  useChat: (...args: unknown[]) => mockUseAIChatFn(...args),
}));
...
describe("ChatContext", () => {
  ...
  beforeEach(() => {
    ...
    mockUseAIChatFn.mockReturnValue(mockUseAIChat);
  });
  ...
  test("initializes with project ID and messages", () => {
    ...
    mockUseAIChatFn.mockReturnValue({
      ...mockUseAIChat,
    ...
    expect(mockUseAIChatFn).toHaveBeenCalledWith({
      api: "/api/chat",
      initialMessages,
      body: {
  ...
  test("handles tool calls", () => {
    ...
    mockUseAIChatFn.mockImplementation((config: any) => {
